...
void TextRenderer::LoadFont(std::string filename, GLuint fontsize) 
{
    ...
    // Load ttf font.
    FT_Face font_face;
    if (FT_New_Face(ft, filename.c_str(), 0, &font_face)) {
        fprintf(stderr,"Could not load true-type font %s\n", filename.c_str());
    }
    
    FT_Set_Pixel_Sizes(font_face, 0, fontsize);
...
void TextRenderer::RenderText(std::string text, GLfloat x, GLfloat y, GLfloat scale, glm::vec4 color)
{
    ...
    for (c = text.begin(); c != text.end(); c++)
    {
        ...
        GLfloat ypos = y + (this->m_characters['H'].Bearing.y - ch.Bearing.y) * scale;  
