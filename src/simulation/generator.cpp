...
namespace Simulation {
    ...
    void Generator::GenerateAllWorlds () {
        ...
        for (int i=0; i < this->universe->getWorldCount(); i++) {
            if (worlds_to_next_system <= 0) {
                if (current_solar_system != 0) { // Init solar system AFTER adding worlds
                    current_solar_system->Init(this->GetNewSolarSystemPosition());
                }
        ...
        if (current_solar_system != 0) { // Init final solar system
            current_solar_system->Init(this->GetNewSolarSystemPosition());
        }
    }
    ...
    glm::vec2 Generator::GetNewSolarSystemPosition() {
        std::uniform_real_distribution<float> position_dist(MIN_XY_WORLDPOS, MAX_XY_WORLDPOS+1);
        glm::vec2 proposed_position;
        bool posNotFound = true;
        int count = 0;
        SolarSystem** solar_systems = this->universe->getSolarSystems();
        
        while (posNotFound && count <= 10) {
            proposed_position = glm::vec2(position_dist(*this->m_mtgen), position_dist(*this->m_mtgen));
            if (this->universe->getSolarSystemCount() == 1) { break; }
            count++;
            for (int i=0; i < this->universe->getSolarSystemCount(); i++) {
                glm::vec2 ss_pos = solar_systems[i]->getPosition();
                if (ss_pos.x <= MAX_XY_WORLDPOS) {
                    if (Vector2Dist(ss_pos, proposed_position) < MIN_SOLARSYSTEM_DISTANCE) {
                        posNotFound = true;
                        break;
                    }
                    posNotFound = false;
        ...
        if (count >= 10) {
            throw NoValidPositionSSException();
        }
